...
const ClientOnlyLoadingIndicator = dynamic(
  () => Promise.resolve(() => <LoadingIndicator />), 
  { ssr: false }
);

const apiKey = '3axw9qwh3k6h';
// const userToken =
...
export default function Home() {
  ...
  if (!myState) {
    return <ClientOnlyLoadingIndicator />;
  }
  ...
  async function getUserToken(userId: string, userName: string) {
    ...
    const user: User = {
      id: userId,
      name: myUser?.firstName || 'Unknown', // Use firstName instead of 
      image: myUser?.imageUrl, // Use Clerk profile image
    };
    setMyState({
      apiKey: apiKey,
      user: user,
